...
class Entity{
public:
	virtual void drawHUD(struct tank*, WarField *, wardraw_t *, const double inverse_rotation[16], void (*grounddraw)(void));
	virtual void cockpitview(struct tank*, WarField *, double (*pos)[3], int *chasecamera);
	virtual void control(struct tank*, WarField*, const input_t *inputs, double dt);
	virtual void destruct(struct tank*);
	virtual int getrot(struct tank*, WarField *, double (*rotationMatrix)[16]);
	virtual void drawCockpit(struct tank*, const WarField *, wardraw_t *);
	virtual int is_warping(struct tank*, const WarField *);
	virtual struct war_field *warp_dest(struct tank*, const WarField *);
	virtual const char *idname(const struct tank*);
	virtual const char *classname(const struct tank*);
	virtual void start_control(struct tank*, WarField*);
	virtual void end_control(struct tank*, WarField*);

	/* bitmask indicating available analog channels. used channels should not be
		addressed to control camera. */
	virtual unsigned analog_mask(const struct tank*, const WarField*);
	...
	double pyr[3];
	double omg[3];
	double rot[4]; /* rotation expressed in quaternion, instead of pyr */
	double mass; /* [kg] */
